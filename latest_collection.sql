select k.school_enabled, k.school_id, k.school_name, max(k.esa_enabled), max(k.esa_state), max(k.esa_id), max(k.esa_name), max(k.dist_enabled), max(k.dist_id), max(k.dist_name), max(k.school_count), max(k.active), max(k.pre_a), max(k.post_a)
from
...
left join survey_responses pre_aug_sr on pre_aug_sr.organization_id = s.id and pre_aug_sr.submitted_at < '2013-08-01' and pre_aug_sr.survey_template_id in (3,4,5)
left join survey_responses post_aug_sr on post_aug_sr.organization_id = s.id and post_aug_sr.submitted_at > '2013-08-01' and post_aug_sr.survey_template_id in (3,4,5)
left join questionnaire_responses qr on  qr.organization_id = s.id and qr.submitted_at < '2013-10-09' and qr.survey_template_id in (3,4,5)
...
group by b.school_enabled, b.school_id, b.school_name))as k
group by 1,2,3
