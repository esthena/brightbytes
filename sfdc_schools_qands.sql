select e.enabled, e.state, e.id, e.name, d.enabled, d.id, d.name, dist.school_count, s.enabled, s.id, s.name, max(pre_aug_sr.submitted_at), greatest(max(post_aug_sr.submitted_at), max(qr.submitted_at))--max(post_aug_sr.submitted_at), max(qr.submitted_at)
from survey_responses pre_aug_sr
right outer join organizations s on s.id = pre_aug_sr.organization_id
right outer join survey_responses post_aug_sr on s.id = post_aug_sr.organization_id
right outer join questionnaire_responses qr on s.id = qr.organization_id
...
where pre_aug_sr.submitted_at < date '2013-08-01'
and post_aug_sr.submitted_at > date '2013-08-01'
and post_aug_sr.submitted_at < date '2013-10-09'
and qr.submitted_at < date '2013-10-09'
